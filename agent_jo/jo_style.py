...
CONTINUE_TEMPLATES = [
    "Продолжи работу. Следующие задачи: {tasks}\nКаждый шаг пиши на русском. Статус сохраняй в репозиторий.",
    "Продолжи. {tasks}\nКаждый шаг пиши на русском.",
    "Продолжи работу. {tasks}\nДелай всё сам, одним батчем.",
    "Продолжи работу над проектом. {tasks}\nВ конце — один файл-отчёт: кратко, но всё.",
]
...
def compose(kind: str, ctx: dict, n: int) -> str:
    ...
    # CONTINUE
    tasks = ctx.get("tasks") or "проверь production readiness, найди слабые места, предложи улучшения."
    return CONTINUE_TEMPLATES[n % len(CONTINUE_TEMPLATES)].format(tasks=tasks)
