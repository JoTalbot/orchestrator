...
| 403 recaptcha / 429 | штрафной кулдаун (×2 за каждой неудачей подряд: 1200 → 7200 с) + замедление темпа |
| нужна пауза (ночь, массовые задачи) | `POST /v1/arena/pause {"seconds":3600}` — быстрый 503 + `Retry-After`, арена не трогается |
| модалка «Security Verification» в вкладке / 2 отказа reCAPTCHA подряд | флаг на аккаунт: `security_check_required: true`, автопауза 7200 с, ошибки `security_check` (503/403) вместо выжигания попыток. Снимается вручную (решить v2 в браузере сервера) или первым успешным запросом |
